              ...
              <li key={i} ref={hover.anchorRef} className="author-list-item">
                ...
                {isPost && (
                  <div className={`author-card ${hover.state.currentClass}`}>
                    <div className="author-profile-image">{author.profile_image && <img src={author.profile_image} alt={author.name} />}</div>
                ...
                <Link href={url}>
                  {profileImg && nextImages.feature ? (
                    <a className={`${(isPost && `author`) || `static`}-avatar`} aria-label={author.name}></a>
